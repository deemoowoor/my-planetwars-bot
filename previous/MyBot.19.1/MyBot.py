...
def findResourcesForRescue(dp, splanets, eplanets, fleets, rescuePlan):
    ...
    for sp in splanetsnodp:
        ...
        safeLimits[sp.ID()] = safeDispatchLimit(sp, closestFP, closestEP, splanetsnodp, eplanetsnodp, fleets)
    ...
    for turn, nships in rescuePlan:
        ...
        for sp in splanetsnodp:
            ...
            if futureGrowth < 0:
                futureGrowth = dp.GrowthRate() * (turn - sp.DistanceTo(dp))
            
            safeLimit = min(safeLimits[sp.ID()], sp.NumShipsDispatch())
            
            if safeLimit <= 0 or safeLimit < -futureGrowth or (distance - turn) > (distance / 3.0):
                continue
            
            dispatch = min(nships_left - futureGrowth, safeLimit)
            
            nships_left -= dispatch + futureGrowth
...
def safeDispatchLimit(sp, closestfp, closestep, mplanets, eplanets, fleets, haveDebug=False):
    ...
    calcDefenceStrength = lambda p: p.CombineIncomingFleets(turnlimit=closestEnemyDistanceToSP)
    
    epos = min(map(calcOffencePotentialVsSP, eplanets))
    ...
    spgds = planetGroupDefenceStrength(sp, mplanetsnosp, fleets, closestEnemyDistanceToSP, haveDebug=haveDebug)
    spgds += spds
    
    if haveDebug:
        debug("     Check safe dispatch limit for", sp.ID(), "spds:", spds, "spgds:", spgds, \
        "epos:", -epos, "epgos:", -epgos, "Final:", spgds, max_eos, '=', spgds + max_eos, " of sp", sp.NumShipsDispatch())
    ...
    limit = min(spgds + max_eos, sp.NumShipsDispatch())
...
def getDispatchSources(mplanets, eplanets, fleets, myCapturedSoon, enemyCapturedSoon):
    ...
    for sp in mplanets:
        ...
        attacklimit = safeDispatchLimit(sp, sp.ClosestFriendly(), sp.ClosestEnemy(), mplanets, eplanets, fleets)
...
def performPlannedOffence(pw, dplanets, eplanets, mplanets, mfleets, fleets):
    ...
    for dp,dprating in dplanets_ratings:
        ...
        for sp, limit, dpdp, proxy in dispatchplan:
            ...
            dpds_after,dpowner_after = dp.CombineIncomingFleets(ff=ffleets, turnlimit=min(distance, closestEnemyDistanceToDP, closestFutureEnemyDistanceToDP))
            
            need = dpdp + 1
            
            if dpowner != 2:
                dpgds_after = planetGroupDefenceStrength(dp, mplanetsnodp, fleets, closestEnemyDistanceToDP, ff=ffleets)
            ...
            debug("A:", sp.ID(), ">", dp.ID(), "<>", distance, "limit:", limit, "min need:", dpdp + 1)
    ...
    planAttackVectors(dispatchPlans, sourcesUsedShips, sourcesWaiting, attackPlans)
    sendAttacks(pw, attackPlans)
    sendAttackReinforcements(pw, sourcesWaiting, sources, sourcesUsedShips, mplanets)
    
def planAttackVectors(dispatchPlans, sourcesUsedShips, sourcesWaiting, attackPlans):
    ...
    for dp, sp, attacklimit, dispatch, proxy, dprating in dispatchPlans:
        ...
        if needmore <= 0:
            ...
            #dpReserveDict[dp.ID()] += dispatch
            continue

        if dp.Owner() == 2:
            needmore += dp.GrowthRate() * (needmore / sp.GrowthRate())
        ...
        for odp, osp, oattacklimit, odispatch, oproxy, odprating in dispatchPlans:
            ...
            oneedmore = float(odispatch - min(oattacklimit - oused, osp.NumShipsDispatch() - oused))
            
            if dp.Owner() == 2:
                oneedmore += odp.GrowthRate() * (oneedmore / osp.GrowthRate())
            
            odpROI = (odprating - oneedmore / osp.GrowthRate())
            
            if odpROI - dpROI > 0:
                #debug("     ", sp.ID(), ">", dp.ID(), "found a better attack destination:", odp.ID(), dpROI, 'vs', odpROI)
                foundBetter = True
                break
        
        # reserve ships for a future attack
        reserve = min(dispatch, available)
        if not foundBetter and needmore / sp.GrowthRate() < 200.0:
            debug("      Reserving for", sp.ID(), ">", dp.ID(), ":", reserve, "of", available, \
            "steps to grow:", needmore / sp.GrowthRate(), 'needtotal:', dispatch, 'rating:', dprating, 'ROI:', dpROI)
...
def sendAttackReinforcements(pw, sourcesWaiting, sources, sourcesUsedShips, mplanets):
    ...
    for rdp, rddp, rdpneed in sourcesWaiting:
        for rspid, (rsp, attacklimit, fproxies) in sources.items():
            ...
            if leftover > 0:
                proxy = findBestProxy(rsp, rdp, mplanets + fproxies)
...
def sendAttacks(pw, attackPlans):
    ...
    for dp, sp, attacklimit, dispatch, proxy in attackPlans:
        ...
        debug(">>>>>>>> ATTACKING:", sp.ID(), ">>", proxy.ID(), ">>", dp.ID(), \
            ...
            tripdistance , "have:", sp.NumShipsDispatch(), \
...
def estimatePlanetOffensiveThreat(sp, dp, fleets, noGrowth = False, turnlimit=sys.maxint):
    ...
    #limit = min(distance * 2, turnlimit) # set upper limit to double of the distance
    pstrength, powner = sp.CombineIncomingFleets(turnlimit=turnlimit)
    
    incomingFleets = filter(lambda f: f.TurnsRemaining() < turnlimit, sp.IncomingFleets())
    noGrowthTurns = max(incomingFleets, key=lambda f: f.TurnsRemaining()).TurnsRemaining() \
        if len(incomingFleets) > 0 else 0
    pstrength -= growth * noGrowthTurns
    
    pstrength -= growth * distance
    pstrength = -pstrength if sp.Owner() == 2 else pstrength
    return pstrength
