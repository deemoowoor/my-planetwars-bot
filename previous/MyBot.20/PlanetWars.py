...
class Planet:
    ...
    def CombineIncomingFleets(self, ff=[], turnlimit=sys.maxint, haveDebug=False):
        ...
        if not ff and getattr(self, '_incomingfleetscombined', None):
            if turnlimit in self._incomingfleetscombined:
                ...
                if owner == 0:
                    self._incomingfleetscombined[turnlimit] = (closeststr, owner)
                else:
                    self._incomingfleetscombined[turnlimit] = (closeststr + self._growth_rate * (turnlimit - closest), owner)
        
        gr = self._growth_rate
        powner = self._owner
        pstrength = 0
        ...
        if haveDebug:
            debug("   final:", pstrength, powner, "@", turnlimit, ":", self._incomingfleetscombined);
        ...
        if turnlimit not in self._incomingfleetscombined:
            ...
            if owner == 0:
                ...
                self._incomingfleetscombined[turnlimit] = (closeststr + gr * (turnlimit - closest), owner)
