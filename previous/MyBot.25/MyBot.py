...
def defendOwnPlanets(pw):
    ...
    for dpid, defleets in dpfleets:
        ...
        rescuePlanets = createRescuePlan(dp, fleets, defleets, 0, turnlimit, splanets, eplanets)
...
def createRescuePlan(dp, fleets, defleets, turndelta, turnlimit, splanets, eplanets):
    ...
    for turn in defleets_turns:
        ...
        dpds_turn,dpowner_turn = dp.CombineIncomingFleets(ff=fanthom_fleets, turnlimit=turn)

        if turndelta > 0:
            dpds_turn,dpowner_turn = dp.CombineIncomingFleets(ff=fanthom_fleets, turnlimit=turn+turndelta)
            if dpowner_turn == 1:
                debug("  @turn:", turn+1, " -- rescued. Skipping.")
                continue
        
        debug("  @turn:", turn, "^", dpds_turn, 'own:', dpowner_turn, 'turnlimit:', turnlimit)
        
        if dpowner_turn != 1:
            numships = dpds_turn + turndelta * dp.GrowthRate() + turndelta
            
            # CHECK IF THERE ARE RESOURCES AVAILABLE HERE RIGHT AWAY
            dispatch, curRP = findResourcesToBuildRescueFleet(dp, turn, numships, splanetsnodp, safeLimits, turnlimit)
            ...
            rescuePlanets.append((turn, dispatch, curRP))
...
def safeDispatchLimit(sp, closestfp, closestep, mplanets, eplanets, fleets, haveDebug=False):
    ...
    closestFriendlyDistanceToSP = closestfp.DistanceTo(sp) if len(mplanets) > 0 else 0
    ...
    maxeos = min(epgos, epos)
    distanceEnemyToSP = closestEnemyDistanceToSP - 1 if epgos > epos else ep.DistanceTo(sp) - 1
...
def planAttackVectors(dispatchPlans, sourcesUsedShips, sourcesWaiting, attackPlans, eplanets):
    ...
    for dp, sp, attacklimit, dispatch, proxy, dprating in dispatchPlans:
        ...
        for odp, osp, oattacklimit, odispatch, oproxy, odprating in dispatchPlans:
            ...
            #TODO: check, if difference between distances will cover the growth 
            # of the closer source to provide a better attack vector
            #if oneedmore 
        ...
        # TODO: check 0 division
        if sp.GrowthRate() > 0 and needmore / sp.GrowthRate() < 50.0:
        ...
        debug("      NOT reserving for", sp.ID(), ">", dp.ID(), 'avail:', available, \
            "needmore:", needmore, 'needtotal:', dispatch, 'growth:', sp.GrowthRate(), \
            'rating: %.2f' % dprating, 'ROI: %.2f' % dpROI)
...
def planetDistanceAndGrowthBlob(planet, planets):
    ...
    return sum(map(lambda p: float(p.GrowthRate()) / (planet.DistanceTo(p) + 1e-300), planetsnodp)) / float(len(planetsnodp))
...
def planetRating(planet, mplanets, eplanets):
    ...
    mdist = 1e-300
    if len(mplanets) > 0:
        mdist = float(planetDistanceBlob(planet, filter(lambda p: p.ID() != planet.ID(), mplanets))) + 1e-300
    
    rating = 0.0
    if planet.Owner() == 2:
        rating = -mdist + planet.GrowthRate()
    else:
        rating = -(mdist + float(planet.NumShips()) / (float(planet.GrowthRate()) + 1e-300))
